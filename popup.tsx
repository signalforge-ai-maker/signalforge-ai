...
function getStrategyDetail(language: Language, strategy: StrategyType) {
  ...
  const details: Record<
    StrategyType,
    { description: string; entryBias: string; risk: string }
  > = {
    momentum: {
...
function CandlestickChart({
  ...
  const priceYFor = (price: number) =>
    padding + ((max - price) / range) * (priceHeight - padding * 2);
  ...
  const ma7Path = buildPath(
    indicators.ma7.map((point, index) => ({
  ...
  const ma25Path = buildPath(
    indicators.ma25.map((point, index) => ({
      x: padding + index * xStep,
      y: point.value === null ? 0 : priceYFor(point.value),
      value: point.value,
    })),
  );
  const rsiPath = buildPath(
    indicators.rsi.map((point, index) => ({
      x: padding + index * xStep,
  ...
  const lastRsi = indicators.rsi.findLast(
    ...
    <div className="chart-frame">
      ...
      >
        ...
        <line
          ...
          x1={padding}
          x2={width - padding}
          y1={priceHeight - padding}
          y2={priceHeight - padding}
        />
        <line
          className="support-line"
          x1={padding}
          x2={width - padding}
          y1={priceYFor(indicators.support)}
          y2={priceYFor(indicators.support)}
        />
        <line
          className="resistance-line"
          x1={padding}
          x2={width - padding}
          y1={priceYFor(indicators.resistance)}
          y2={priceYFor(indicators.resistance)}
        />
        <text
          className="chart-level-label support"
          x={width - padding}
          y={priceYFor(indicators.support) - 3}
        >
          S
        </text>
        <text
          className="chart-level-label resistance"
          x={width - padding}
          y={priceYFor(indicators.resistance) + 10}
        >
          R
        </text>
        {rows.map((row, index) => {
          ...
          const volumeBarHeight = Math.max(
            (row.volume / maxVolume) * volumeHeight,
            1,
          );
        ...
        <line
          className="rsi-guide overbought"
          x1={padding}
          x2={width - padding}
          y1={rsiYFor(70)}
          y2={rsiYFor(70)}
      ...
      {last ? (
        <div className="chart-price-row">
          <span>
            Range ${formatPrice(String(min))} - ${formatPrice(String(max))}
          </span>
          <strong
            className={last.close >= last.open ? "change up" : "change down"}
          >
...
function Popup() {
  ...
  const strategySignal = useMemo(
    () => getTopSignal(rows, botSettings.strategyType),
    [botSettings.strategyType, rows],
  );
  const localizedStrategyDetail = getStrategyDetail(
    language,
  ...
  const selectedSignal = useMemo(() => {
    ...
    return selectedRow
      ? scoreMarketRow(selectedRow, botSettings.strategyType)
      : undefined;
  ...
  const chartIndicators = useMemo(
    () => (klines.length ? calculateChartIndicators(klines) : undefined),
    [klines],
  );
  ...
  useEffect(() => {
    ...
    chrome.storage.local.get(STORAGE_KEY, (result) => {
      const saved = result[STORAGE_KEY] as
        | Partial<PersistedBotState>
        | undefined;
      ...
      if (
        saved?.klineInterval &&
        klineIntervals.includes(saved.klineInterval)
      ) {
      ...
      if (saved?.riskSettings) {
        setRiskSettings((settings) => ({
          ...
          executionMode: normalizeExecutionMode(
            saved.riskSettings?.executionMode,
          ),
        }));
      }
      ...
      if (Array.isArray(saved?.orderHistory))
        setOrderHistory(saved.orderHistory.slice(0, 6));
      setStorageReady(true);
    });
  }, []);
  ...
  const currentPositionRow = paperPosition
    ...
    : undefined;
  const currentPositionPrice = Number(
    currentPositionRow?.lastPrice ?? paperPosition?.entryPrice ?? 0,
  );
  const positionValue = paperPosition
    ? paperPosition.quantity * currentPositionPrice
    : 0;
  const positionCost = paperPosition
    ? paperPosition.quantity * paperPosition.entryPrice
    : 0;
  ...
  const riskCheck = useMemo<RiskCheck>(() => {
    const closedOrders = orderHistory.filter(
      (order) => order.side === "close" && typeof order.pnl === "number",
    );
    const realizedPnl = closedOrders.reduce(
      (sum, order) => sum + (order.pnl ?? 0),
      0,
    );
    ...
    if (
      (configuredBotPlan?.allocationPercent ?? 0) >
      ...
      return {
        ...
        reason:
          language === "zh"
            ? "计划仓位超过最大仓位限制。"
            : "Planned position is above the max position limit.",
        realizedLossPercent,
        consecutiveLosses,
      };
    }
    ...
    if (riskSettings.executionMode === "alert-only") {
      return {
        ...
        reason:
          language === "zh"
            ? "允许生成信号，但禁止执行订单。"
            : "Signals are allowed, but order execution is disabled.",
        realizedLossPercent,
        consecutiveLosses,
      };
    }
    ...
    if (riskSettings.executionMode === "manual-confirm") {
      return {
        ...
        reason:
          language === "zh"
            ? "任何模拟入场前都需要手动点击确认。"
            : "Manual click is required before any paper entry.",
        realizedLossPercent,
        consecutiveLosses,
      };
    }
    ...
    return {
      ...
      reason:
        language === "zh"
          ? "当前风控允许自动模拟执行。"
          : "Risk limits allow automated paper execution.",
      realizedLossPercent,
      consecutiveLosses,
    };
  }, [
    configuredBotPlan?.allocationPercent,
    language,
    orderHistory,
    riskSettings,
  ]);
  ...
  const updateBotSetting = useCallback(
    (
      ...
      value: number | boolean | MarketType | StrategyType,
    ) => {
      setBotSettings((settings) => ({
        ...settings,
        [key]:
  ...
  const addOrderEvent = useCallback(
    (event: Omit<OrderEvent, "id" | "timestamp">) => {
  ...
  const openPaperPosition = useCallback(
    (reason = "Manual paper entry", source: "manual" | "auto" = "manual") => {
      if (
        !configuredBotPlan ||
  ...
  useEffect(() => {
    if (!botSettings.autoPaperTrading || !configuredBotPlan || paperPosition)
      return;
  ...
  const loadMarketData = useCallback(async () => {
    ...
    try {
      ...
      if (
        tradeTarget !== "auto" &&
        !marketRows.some((row) => row.symbol === tradeTarget)
      ) {
        setTradeTarget("auto");
      }
      setSource("live");
  ...
  const loadKlines = useCallback(async () => {
    ...
    try {
      ...
      setChartState("Error");
      setChartError(
        language === "zh" ? "K线数据不可用。" : "Kline data is unavailable.",
      );
    }
  ...
  const loadNewsSentiment = useCallback(async () => {
    ...
    try {
      ...
      setNewsError(
        language === "zh"
          ? "新闻源离线，正在显示市场备用情绪。"
          : "News source is offline. Showing market-only fallback.",
      );
    }
    ...
    <main className="shell">
      ...
      <section className="control-grid" aria-label="Trading mode controls">
        ...
        <label htmlFor="marketTypeSelect">
          ...
          <select
            ...
            onChange={(event) =>
              updateBotSetting("marketType", event.target.value as MarketType)
            }
          >
      ...
      <section className="signal-panel">
        <div className="section-heading">
          ...
          <span className={`badge ${badge.mode}`}>
            {getSignalBadgeLabel(language, badge.label)}
          </span>
        </div>
        <article className="signal-card">
          {topSignal ? (
            <>
              ...
              <p className="reason">
                {buildLocalizedReason(language, topSignal, topSignal.score)}
              </p>
      ...
      <section className="chart-panel">
        <div className="section-heading">
          ...
          <span className="muted">
            {chartSymbol ? formatSymbol(chartSymbol) : "--"}
          </span>
        </div>
        ...
        {chartState === "Ready" && klines.length && chartIndicators ? (
          ...
          <div className="chart-placeholder">
            {chartState === "Loading"
              ? t(language, "candlesLoading")
              : chartError || t(language, "noCandles")}
          </div>
        )}
      </section>
      ...
      <section className="news-panel" aria-label="News sentiment">
        <div className="section-heading">
          ...
          <span
            className={`badge sentiment ${getSentimentClass(newsSentiment?.label)}`}
          >
            {newsState === "Loading"
              ? t(language, "scanning")
              : getSentimentLabel(language, newsSentiment?.label)}
          </span>
        </div>
        <article className="news-card">
          {newsSentiment ? (
            <>
                ...
                <div>
                  <span>
                    {newsSentiment.source === "news"
                      ? t(language, "headlineScore")
                      : t(language, "marketScore")}
                  </span>
              ...
              {newsState === "Error" ? (
                <p className="news-warning">{newsError}</p>
              ) : null}
            </>
          ) : (
            <p className="placeholder">
              {newsState === "Loading"
                ? t(language, "scanningHeadlines")
                : newsError}
            </p>
          )}
        </article>
        ...
        <article className="risk-card">
          <div className="risk-mode-row">
            <div>
              ...
              <strong>
                {getExecutionModeLabel(language, riskSettings.executionMode)}
              </strong>
            </div>
            <span className="risk-lock">
              {t(language, "realTradingLocked")}
            </span>
          </div>
          <div className="mode-selector" aria-label="Execution mode">
            {[
              ...
              <button
                ...
                type="button"
                className={
                  riskSettings.executionMode === mode.value
                    ? "mode-button active"
                    : "mode-button"
                }
                onClick={() =>
          ...
          <div className="risk-grid">
            <label>
              <span>
                {language === "zh" ? "每日最大亏损 %" : "Daily max loss %"}
              </span>
              <input
                type="number"
                min="0.5"
          ...
          <div className={`risk-check ${getRiskCheckClass(riskCheck.status)}`}>
            ...
            <div className="risk-check-metrics">
              <span>
                {language === "zh" ? "亏损" : "Loss"}{" "}
                {riskCheck.realizedLossPercent.toFixed(2)}%
              </span>
              <span>
                {t(language, "streak")} {riskCheck.consecutiveLosses}
              </span>
            </div>
          </div>
        </article>
          ...
          {configuredBotPlan ? (
            <>
              <div className="signal-title">
                ...
                <span
                  className={
                    configuredBotPlan.action === "risk-off"
                      ? "change down"
                      : "score"
                  }
                >
              ...
              <div className="strategy-strip">
                <span>{t(language, "strategy")}</span>
                <strong>
                  {getStrategyLabel(language, configuredBotPlan.strategy)}
                </strong>
              </div>
              ...
              <div className="bot-grid">
                ...
                <div>
                  ...
                  <strong>
                    ${formatPrice(String(configuredBotPlan.stopLoss))}
                  </strong>
                </div>
                <div>
                  <span>{t(language, "target")}</span>
                  <strong>
                    ${formatPrice(String(configuredBotPlan.takeProfit))}
                  </strong>
                </div>
              </div>
              <div className="bot-settings" aria-label="Paper bot settings">
                {botSettings.marketType === "futures" ? (
                  <label>
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        updateBotSetting("leverage", Number(event.target.value))
                      }
                    />
                  </label>
                ) : null}
                ...
                <label className="toggle-setting">
                  <input
                    ...
                    onChange={(event) =>
                      updateBotSetting("autoPaperTrading", event.target.checked)
                    }
                  />
                  <span>Auto</span>
                </label>
              ...
              <div className="paper-account">
                ...
                {paperPosition ? (
                  <div className="metric-row">
                    <span>
                      {language === "zh"
                        ? `${paperPosition.openedAt} 后盈亏`
                        : `PnL since ${paperPosition.openedAt}`}
                    </span>
                    <strong
                      className={paperPnl >= 0 ? "change up" : "change down"}
                    >
                ...
                {paperPosition ? (
                  <div className="metric-row">
                    ...
                    <strong>
                      ${formatPrice(String(paperPosition.stopLoss))} / $
                      {formatPrice(String(paperPosition.takeProfit))}
                    </strong>
                  </div>
                ) : null}
              </div>
      ...
      <section>
        <div className="section-heading">
          ...
          <span className="muted">
            {orderHistory.length} {language === "zh" ? "条记录" : "events"}
          </span>
        </div>
            ...
            orderHistory.map((order) => (
              <article key={order.id} className="history-row">
                <div>
                  ...
                  <span>
                    {order.reason} {language === "zh" ? "时间" : "at"}{" "}
                    {order.timestamp}
                  </span>
                  <span>
                    {getMarketLabel(language, order.marketType, order.leverage)}
                  </span>
                </div>
                <div>
      ...
      <section>
        <div className="section-heading">
          ...
          <span className="muted">
            {rows.length} {language === "zh" ? "个资产" : "assets"}
          </span>
        </div>
